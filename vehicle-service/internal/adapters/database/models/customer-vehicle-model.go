...
type CustomerVehicle struct {
	...
	OwnedFrom  time.Time                 `json:"owned_from" gorm:"column:owned_from;type:date"`
	OwnedTo    *time.Time                `json:"owned_to" gorm:"column:owned_to;type:date"`
	Status     constants.OwnershipStatus `json:"status" gorm:"column:status;type:varchar;size:20"`
